...
const EditMeeting = () => {
  ...
  const bookedTimesForDate = date && selectedParticipantIds.length > 0
    ? getBookedSlotsForParticipants(meetings, date, selectedParticipantIds, id)
    : [];
      ...
      <form onSubmit={handleUpdate} className="product-form edit-form">
        ...
        <div className="form-group">
          ...
          <div className="mode-toggle">
            <button type="button" className={`mode-btn ${mode === 'online' ? 'active' : ''}`} onClick={() => setMode('online')}>
              🌐 Online
            </button>
            <button type="button" className={`mode-btn ${mode === 'physical' ? 'active' : ''}`} onClick={() => setMode('physical')}>
              📍 Physical
            </button>
          </div>
        </div>
        ...
        <div className="form-actions">
          <button type="submit" className="btn-primary">Update Meeting</button>
